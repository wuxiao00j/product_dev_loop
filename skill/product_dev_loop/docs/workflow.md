...
当用户输入属于“继续 / 继续项目 / 接手项目 / 跟进项目 / 给我下一步 / 继续喂 Cursor”这类意图时，不要直接进入 `Build Delegation`。
...
- 有歧义时先向用户补问，不默认推进
- 一次只补问最影响推进的那个问题
- 默认动作类型、tool 调用时机、记录策略、最终输出协议，以 `docs/default_collaboration_rules.md` 为准
...
出现以下任一情况时，不默认继续 rewrite，也不默认继续派单：
...
3. 写明：
   ...
   - 预期结果
   - 失败判定
4. 如果执行器只是声称“已完成”，但证据不足，要明确标为“需重点验证”
5. 如果有已知限制、环境要求、隐藏入口、`stub / mock / 假数据 / 未接线`，也要明确写出
6. 测试描述必须让用户可以直接按步骤执行，而不是看代码说明
7. 生成完 `TEST_GUIDANCE.md` 后，应把其中最关键的测试步骤直接输出给用户，而不是只提醒“请去测试”
...
1. 先判断输入更像：执行器回执、用户测试反馈，还是外部阻塞说明
2. 如果是执行器回执：先压缩成关键事实，不直接照抄原文
3. 如果是用户测试反馈：先分类为实现缺陷 / PRD 未达标 / 新需求 / 隐性偏好 / 外部阻塞 / 本轮通过
4. 如果是外部阻塞说明：优先输出阻塞提醒，而不是继续写下一轮 prompt
5. 把实现事实、关键文件、风险和特殊状态写入 `IMPLEMENTATION_NOTES.md`
6. 在 `PROGRESS.md` 写一段 1 到 2 句短摘要
7. 对照 PRD 判断：
   - 已完成什么
   - 未完成什么
   - 是否闭环
   - 是否存在 `stub / mock / 假数据 / 隐藏入口 / 未接线`
8. 从回执里提炼用户现在最需要验证的测试点，生成或更新 `TEST_GUIDANCE.md`
9. 再判断当前更适合：
   - 继续派单
   - 进入用户测试
   - 等待用户确认
   ...
   - 进入 `CHANGE_PROPOSALS.md`
   - 进入下一阶段
10. 在 `PROJECT_STATE.md` 更新当前阶段、当前结论、阻塞和下一步
11. 如果出现范围变化，先写 `CHANGE_PROPOSALS.md`
12. 如果确认了新的隐性要求，写 `IMPLICIT_REQUIREMENTS.md`
13. 只有在反馈被判断为可继续执行的实现问题后，才生成下一轮提示词
14. 如需生成下一轮提示词，可调用 `product_dev_loop_tool skeleton / gate / contract` 辅助判断 rewrite_more / re_gate / pass
...
- `TEST_GUIDANCE.md`：写用户可读测试说明，解决“现在该怎么测”
- `TEST_RESULTS.md`：写实际测到的问题和通过情况，记录“实际测到了什么”
...
内部可以做完整分析，但对用户的默认输出应优先收敛成以下 4 类之一：

1. 给 Cursor / Codex 的下一段提示词
2. 给用户的测试说明
3. 给用户的阻塞提醒
4. 给用户的最小澄清问题
